...
public class ProblemDiagram extends Application implements EventHandler<ActionEvent> {
	...
	public ProblemDiagram(String nameToSet){
		this.name = nameToSet;
	}
	...
	public void start(Stage primaryStage) {
		...
		primaryStage.setTitle("ProblemDiagram");
	    ...
	    Pane domainSandboxLayout = new Pane();
	    topBarLayout.setTop(menuBar);
	    topBarLayout.setCenter(buttonMenu);
	    topBarLayout.setPrefSize(1000, 65);
	    
        domainSandboxLayout.autosize();
        ...
        bottomTextRegion.setPrefSize(1000, 100);
        lastEventTextField.setPrefSize(1000, 25);
		...
		Domain machineDomain = new Domain("Machine Domain");
		DomainPane machineDomainPane = new DomainPane(470, 260, machineDomain);
		machineDomainPane.setBackground(new Background(new BackgroundFill(Color.web("RED"), CornerRadii.EMPTY, Insets.EMPTY)));
		machineDomainPane.setOnMouseDragged(e -> {});
		domainSandboxLayout.getChildren().add(machineDomainPane);
		this.problemFrame.addDomain(machineDomain);
		...
		TextInputDialog requirementCreationDialog = new TextInputDialog("Requirement X");
		requirementCreationDialog.setTitle("Requirement Creation Dialogue");
		requirementCreationDialog.setHeaderText("Requirement Creation Dialog");
		requirementCreationDialog.setContentText("Please enter requirement name");

		//REMOVE Requirement DIALOG
		Alert removeRequirementDialog = new Alert(AlertType.CONFIRMATION);
		removeRequirementDialog.setTitle("Remove Requirement Dialog");
		removeRequirementDialog.setHeaderText("Remove Requirement?");
		removeRequirementDialog.setContentText("Are You Sure You Want To Remove This Requirement?");
		
		//REMOVE RequirementReference DIALOG
		Alert removeRequirementReferenceDialog = new Alert(AlertType.CONFIRMATION);
		removeRequirementReferenceDialog.setTitle("Remove Requirement Reference Dialog");
		removeRequirementReferenceDialog.setHeaderText("Remove Requirement Reference?");
		removeRequirementReferenceDialog.setContentText("Are You Sure You Want To Remove This Requirement Reference?");
		...
		quickStartButton.setOnAction(e -> {
			...
			Optional<ButtonType> removeResult = removeRequirementDialog.showAndWait();
		...
		helpButton.setOnAction(e ->{
			...
			Optional<ButtonType> removeResult = removeRequirementReferenceDialog.showAndWait();
			if (removeResult.get() == ButtonType.OK){
			    // ... user chose OK
			} else {
			    // ... user chose CANCEL or closed the dialog
		...
		hideToolbarButton.setOnAction(e -> {
			...
			removeRequirementReferenceDialog.showAndWait();
		...
		domainSandboxLayout.setOnMouseClicked( e -> {
			...
			double yOfClick = e.getSceneY()-65;	//this handles the offset of the drawing pane in the pane
			
			//if there is already a domain at this position, we want to edit it, not create a new one
			if(xOfClick > 0){
				
				Optional<String> nameOfRequirement = requirementCreationDialog.showAndWait();
				
				Requirement requirementCreated = new Requirement(nameOfRequirement.toString());
				
				if(nameOfRequirement.isPresent()){
					
					RequirementPane requirementPane = new RequirementPane(xOfClick, yOfClick, requirementCreated);
					domainSandboxLayout.getChildren().add(requirementPane);
					this.problemFrame.addRequirement(requirementCreated);
				...
				if(debug){
					System.out.println("Context Frame Requirement List: ");
					for(Requirement requirement : this.problemFrame.requirements){
						System.out.println(requirement.getName());
					}
				}
				
			}
			//end the click handler
